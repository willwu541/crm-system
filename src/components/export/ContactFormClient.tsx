...
export function ContactFormClient({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const createdContactId = json.data?.id;
      if (!contactId && createdContactId && files.length > 0) {
        for (const file of files) {
          const fd = new FormData();
          fd.append("file", file);
          fd.append("entityType", "export_contact");
          fd.append("entityId", createdContactId);
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {form.whatsapp.trim() ? (
          <div className="sm:col-span-2 rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-800">
            有 WhatsApp 号码后，请先联系上客户。留下沟通记录之后，才会进入维护提醒。
          </div>
        ) : null}
        <div>
